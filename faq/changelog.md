...
You know the alert you stopped reading. The motion sensor by the side gate that goes off for the cat, the delivery driver and a branch in the wind, until one week you mute it — and then it is no use on the night it matters. **3.9.0 lets you say "only tell me if it's still happening."** Someone by the gate for ten minutes is worth knowing about; someone walking past is not, and now your home can tell the difference. **The same change means one automation can look after every window in the house** instead of one automation per window, and it still tells you which window. There is also **one tidy page for everything your sensors measure**, your **charts can print the numbers on the bars**, and if you have hooked up ChatGPT or Claude, it now knows which things it can just do and which it should ask you about first. [app.chirpwireless.io](https://app.chirpwireless.io)

***

#### What's in This Release

* **"Only if it's still happening"** — Make an automation wait before it does anything. The freezer door open for thirty seconds is you getting the peas out; open for ten minutes it has not shut. Same sensor, and now your home knows the difference.
* **One automation for every window** — A trigger holds its own list of sensors, so nine windows need one automation instead of nine. Each is still watched separately, and the alert can name the one that set it off.
* **Everything your sensors measure, on one page** — Search it, filter it, sort it, and add something new in a single box. No more hunting through three tabs.
* **Numbers printed on your bar charts** — Read the figure straight off the bar instead of squinting across at the scale, and repeat the reading underneath the graph so a small widget still makes sense.
* **Your AI app knows what's safe** — Everything it can do now says whether it only looks or actually changes something, so it answers questions freely and asks first before touching your house.
* **Smoothing things out** — Sensor photos save properly, the Activity Log lists permission changes again, a widget number goes back to changing color with its conditions, log retention now matches your plan, the helper copes with longer stretches of history, a sensor that only reports once a day is no longer called offline by lunchtime, and a pile of layout and wording fixes.

***

**Only if it's still happening**

Most automations react the instant a sensor speaks. That is exactly what you want for a leak — water on the basement floor is worth knowing about the same second. It is exactly what you do not want for almost everything else.

A trigger is the new way to say *"and it stayed that way."* Pick what you are watching, then choose **Only if it lasts** and set a time — seconds, minutes, hours or days. Nothing happens until the situation has held that long. Ten minutes is what a new trigger starts on, because that is what most people want it for.

The bathroom humid for half an hour is somebody's shower. Humid all day means it is not clearing, and that is how you end up with mould. The garage door open for a few minutes is normal; open all night is not. None of that needs a cleverer sensor — it needed a way to tell your home that time is part of the problem.

Battery sensors do not chatter, so if nothing new arrives during the wait the situation simply carries on counting rather than falsely clearing. And if the all-clear needs to settle too, **Clear by a separate condition** gives the other direction its own wait, so a motion sensor going quiet for one reading does not count as everyone having gone.
...
**One automation for every window**

Once the idea works on one window you want it on all of them, and that used to mean building the same automation over and over — nine windows, nine copies, nine places to go when you change your mind about the timing.

A trigger keeps its own list of sensors. Describe the situation once, tick the ones it applies to, and that is the job done. A tenth window later is one more tick.

They are still watched **separately**, which is the part that matters: the kitchen window has nothing to do with the landing window, each is timed on its own, and each can set the automation off by itself. Before you save, **How this trigger will run** shows a row per sensor so you can check it is watching what you meant. And because your alert can include the name of the sensor that fired, "a window is open" becomes "the landing window is open" — which is the difference between checking one window and checking nine.
...
What your sensors measure — the names, the units, how each value is stored — used to live across three separate tabs. It is now one list under **Devices → Metrics**.

Type to search it by name. Narrow it by unit, by how the value is stored, or by what kind of measurement it is, ticking as many as you like. Sort by newest to jump back to whatever you just added. Adding and editing happen in the same single box, so there is only one set of fields to learn, and units and names are created or renamed right inside their own dropdowns.

One thing worth knowing: these are shared across your whole home rather than set per sensor, so editing one changes it everywhere it is used. Chirp says so before it does it — and if you only meant to change one sensor, make a new one and point that sensor at it instead.
...
When you are comparing one day against another rather than watching a line drift, working out what each bar actually was means tracking across to the scale and guessing. **Display value on bar** writes the number on the bar. It only appears for bar charts, since a line has no bars to write on.

**Show metrics below** goes with it: the reading is repeated in a line under the graph, which is what keeps a widget readable on a phone, or once you have squeezed it onto a busy dashboard.
...
If you have connected ChatGPT or Claude to your home, it used to see a list of things it could do and no sense of which ones mattered.

Now each one says how it behaves: whether it only looks something up, whether it changes something, and whether it reaches outside your home at all. So your assistant answers "how warm is the nursery?" straight away, and stops to ask before it switches a lamp off or removes a sensor. You do not have to set any of this up — it simply arrives that way, and it comes from the live list of what the server can actually do, so it cannot go out of date.
...
Photos you add to a sensor now save. **The Activity Log lists permission changes again** — changing what someone in your home can see had stopped leaving a record, so the page looked frozen. **A number on a Last Data widget goes back to changing color with its conditions**, which an upgrade had quietly broken. **The Key Vault now describes only the fields it actually uses**, instead of mentioning kinds of sensor Chirp does not support. **How long your device logs are kept now matches your plan**, which it had not been doing. The helper copes with being asked for longer stretches of history instead of saying it is unavailable. A sensor set to report once a day is no longer called offline a few hours later, and the warning now tells you the units it means. Asking for help is a quick pick of bug report, feature request or integration request rather than a blank box. Pretend sensors timestamp their first reading correctly. The screens you have not filled in yet all look the same as each other.
