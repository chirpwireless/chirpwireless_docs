...
> **A reading missing from the list?** **Device metric** only shows number readings. If one you expect is not there, that metric is set up as text (String) or on/off (Boolean). Open [Data Templates](../../devices/data-templates.md), find the metric on the **Metrics** tab, and change its **Type** to Integer or Float — as long as the sensor sends a number.
...
- **Number** — A plain numeric value with its unit. Clean and direct. [→ Number Display](last-data-widget/number.md)
...
- **Is it on or off?** — a door, a light, a leak sensor. A plain [Number display](last-data-widget/number.md) with one color per state is all you need.
- **What is the reading right now?** — a room temperature, an air-quality figure. A [Number display](last-data-widget/number.md) shows the value; a [Gauge display](last-data-widget/gauge.md) also shows whether it is in a comfortable range.
