---
description: "Connect LoRaWAN sensors to Chirp digital twins: device profiles, keys, decoding, measurement mapping, commands and replacement."
---

# LoRaWAN Devices

A **LoRaWAN device** sends small readings over a radio link to a gateway. Chirp's LNS connector receives those messages through its built-in LoRaWAN Network Server. The physical sensor supplies the data; its **digital twin** keeps the name, measurement channels, and retained history you use in dashboards and automation.

For example, a **Garden Soil** device can keep the readings used to plan watering even when you replace the soil-moisture probe. Choose this connection for compatible LoRaWAN hardware; a Wi-Fi or Zigbee sensor needs a different connection path.
...
2. Enter **Device name**, such as **Garden Soil**, optionally add photos, and click **Save**. This creates the digital record before binding hardware. See [shared registration steps](adding-sensors.md) for profile and Application settings.
...
| **Detach physical device** | Immediately removes the hardware binding. Use it for replacement, then bind the new hardware to this twin. Keep the existing measurement rows and remap their incoming keys; see [replacement and history](sensor-details.md). |
...
The template defines meaning, unit and value type; selecting °C does not convert a Fahrenheit reading. **Value** and **Last update** help identify the received field. **Logs** shows retained measurements after mapping. See [mapping controls](sensor-details.md#metrics), [metric templates](data-templates.md), and [payload interpretation](what-your-device-is-sending.md) for all fields and conversion rules.
...
For **Garden Soil**, the same mapped readings can feed a chart and an alarm. If the probe is replaced, retain the twin and its measurement rows, then reconnect the new probe's decoded fields. The retained history remains subject to your subscription's retention period.
...
Continue with [device diagnostics](connection-diagnostics.md) for reception status and processing errors.
