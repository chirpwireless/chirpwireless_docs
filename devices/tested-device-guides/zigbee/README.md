...
That's the normal case. The [Zigbee2MQTT supported devices list](https://www.zigbee2mqtt.io/supported-devices/) covers thousands of devices — is a starting point for checking compatibility. Your bridge must publish a supported MQTT payload, and the fields you need must be mapped in Chirp. The procedure for registering a new Zigbee device is:
...
The only thing that changes between devices is step 1 (pairing varies by manufacturer) and the specific payload keys at step 4 (they differ by device class). Adjust the topic pattern if you configured a different Zigbee2MQTT base topic.
