...
Up to now, Chirp has mostly listened — gathering readings from your sensors and showing them on your dashboards. **Commands** flip that around. They let Chirp *talk back* to your devices, so the same platform that tells you the living room is chilly can also turn the heater on.

With a supported connection and the right command payload, you can flick a smart plug on or off, dim the bedroom lights to 30%, warm a bulb's color temperature for movie night, nudge a thermostat to a new target, or send a setting to almost anything in your home. It works whether your device connects over Zigbee/MQTT or LoRaWAN — you don't have to think about the plumbing.
...
The saved actions belong to the device's [digital twin](../README.md), while the connected hardware carries them out. If you replace a smart plug or garden valve with another model, check its command settings and feedback readings before using the action again. Keeping the same digital device does not make the new model accept the old model's messages.
...
* **LoRaWAN devices** — save the physical connection first. A Class A sensor receives commands after it sends an update, while Class C can listen between updates. Check that the model supports the action you want.
...
1. **A device that can receive commands** — connected over MQTT, a LoRaWAN device that accepts commands, or a [pretend sensor](../pretend-sensors.md) with commands switched on.
